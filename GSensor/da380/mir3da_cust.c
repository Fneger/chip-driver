			      ...
			      unsigned long arg)
{
	...
	case MIR3DA_ACC_IOCTL_GET_COOR_XYZ:
		...
		if (copy_to_user((void __user *)arg, xyz, sizeof(xyz)))
			return -EFAULT;
		break;
	case MIR3DA_ACC_IOCTL_READ_REG:
		if (copy_from_user(ubuffer, argp, sizeof(ubuffer)))
			return -EFAULT;
		if(mir3da_register_read(handle,ubuffer[0],&ubuffer[1]))
		{
			printk("[%s]read reg failed!addr:%d\n",__func__,ubuffer[0]);
			return -EFAULT;
		}
		printk("mir3da_register_read==addr:0x%02x,value:0x%02x\n",ubuffer[0],ubuffer[1]);
		if (copy_to_user(argp, ubuffer, sizeof(ubuffer)))
			return -EINVAL;
		break;

	case MIR3DA_ACC_IOCTL_WRITE_REG:
		if (copy_from_user(ubuffer, argp, sizeof(ubuffer)))
			return -EFAULT;
		if(mir3da_register_write(handle,ubuffer[0],ubuffer[1]))
		{
			printk("[%s]write reg failed!addr:%d, value:%d\n",__func__,ubuffer[0],ubuffer[1]);
			return -EFAULT;
		}
		printk("mir3da_register_write==addr:0x%02x,value:0x%02x\n",ubuffer[0],ubuffer[1]);
		break;
	default:
		return -EINVAL;
	}
