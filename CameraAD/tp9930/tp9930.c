 ...
 *
 * Authors:  Zhouhongbiao <Hongbiao.zhou@phitec.com.cn>
 *
...
#define DUMP_TEST
#ifdef DUMP_TEST
struct sensor_info info_test;
#endif

//#define	TP9930_USE_1080P_2CH
//#define	TP9930_USE_720P_2CH
#define	TP9930_USE_1080P_4CH
#define	TP9930_USE_720P_4CH
...
#if defined(TP9930_USE_720P_4CH) || defined(TP9930_USE_1080P_4CH)
#define DETECT_GPIO_MAX 4
#else
#define DETECT_GPIO_MAX 2
#endif
...
static struct sensor_info *s_info = NULL;
...
static struct regval_list reg_1080p25_4ch[] =
{
    ...
    //video setting
    {0x40, 0x04},
    {0x02, 0x44},	//set 16bit bt656 output
    {0x05, 0x00},
    {0x06, 0x32},
    {0x07, 0x80},	//{0x07, 0xc0},
    {0x08, 0x00},
    ...
    {0x0b, 0xc0},
    {0x0c, 0x03},	//{0x0c, 0x13},
    {0x0d, 0x73},	//decoding format: 1080p25
    {0x10, 0x00},
    {0x11, 0x40},
    {0x12, 0x40},
    {0x13, 0x00},
...
static int sensor_init(struct v4l2_subdev *sd, u32 val)
{
	...
	info->focus_status = 0;
	info->low_speed = 0;
        info->width = HD1080_WIDTH; //HD720_WIDTH;TDebug
        info->height = HD1080_HEIGHT; //HD720_HEIGHT;TDebug
	info->hflip = 0;
	info->vflip = 0;
...
#if defined(TP9930_USE_1080P_2CH) || defined(TP9930_USE_720P_2CH)
    cfg->flags = CLK_POL | CSI_CH_0 | CSI_CH_1;
#elif defined(TP9930_USE_720P_4CH) || defined(TP9930_USE_1080P_4CH)
...
struct semaphore tp9930_rw_lock;

static int tp9930_get_vloss(unsigned int *pVLossST)
{
	int				idx = 0;

	data_type		reg_value = 0x0;
	
	unsigned int	nCurVLoss = 0x0;
	...
	for (idx = 0; idx < 4; idx++)
	{
		sensor_write(cci_drv.sd, 0x40, (data_type)idx);

		sensor_read(cci_drv.sd, 0x1, &reg_value);
...
static int tp9930_set_image(tp9930_image_set *pImageSet)
{
	if (pImageSet == NULL)
	{
		return -1;
	...
	if ((pImageSet->chn >= 4)
		|| (pImageSet->brightness >= 128)
		|| (pImageSet->contrast >= 128)
		|| (pImageSet->saturation >= 128)
		|| (pImageSet->sharpness >= 32))
	{
		printk("[%s, %d] ERR: param error.(ch=%d, brightness=%d, contrase=%d, saturation=%d, sharpness=%d)\n", \
			__FUNCTION__, __LINE__, pImageSet->chn, pImageSet->brightness, \
			pImageSet->contrast, pImageSet->saturation, pImageSet->sharpness);
		return -1;
	}

	sensor_write(cci_drv.sd, 0x40, (data_type)pImageSet->chn);

	sensor_write(cci_drv.sd, 0x10, (data_type)pImageSet->brightness);

	sensor_write(cci_drv.sd, 0x11, (data_type)pImageSet->contrast);

	sensor_write(cci_drv.sd, 0x12, (data_type)pImageSet->saturation);

	sensor_write(cci_drv.sd, 0x13, (data_type)pImageSet->hue);

	sensor_write(cci_drv.sd, 0x14, (data_type)pImageSet->sharpness);
	
	return 0;
}

static int sensor_set_vi_mode(int viMode)
{
    switch (viMode) {
    case TP9930_4X720P_E:
        s_info->fmt 		= &sensor_formats[0];
        s_info->fmt_pt 	= &sensor_formats[0];
        s_info->win_pt 	= &sensor_win_sizes[0];
        s_info->width = HD720_WIDTH;
        s_info->height = HD720_HEIGHT;
        break;
    case TP9930_4X1080P_E:
        s_info->fmt 		= &sensor_formats[1];
        s_info->fmt_pt 	= &sensor_formats[1];
        s_info->win_pt 	= &sensor_win_sizes[1];
        s_info->width = HD1080_WIDTH;
        s_info->height = HD1080_HEIGHT;
        break;
    }
    return 0;
}
			     ...
			     unsigned long arg)
{
	...
	tp9930_register		dev_reg;
	tp9930_vloss		dev_vloss;
	tp9930_image_set	dev_image;
	tp9930_power_set	dev_power;
        tp9930_vi_mode_set      dev_vi_mode;
	
	switch (cmd)
	{
        case TP9930_VI_MODE_SET:
            if (copy_from_user(&dev_vi_mode, argp, sizeof(tp9930_vi_mode_set)))
            {
                return -EINVAL;
            }
            down(&tp9930_rw_lock);
            sensor_set_vi_mode(dev_vi_mode.vi_mode);
            up(&tp9930_rw_lock);
            break;
	case TP9930_READ_REG:
		{
			if (copy_from_user(&dev_reg, argp, sizeof(tp9930_register)))
			{
            	return -EINVAL;
			}
			...
			sensor_read(cci_drv.sd, dev_reg.reg_addr, &dev_reg.value);
			
			up(&tp9930_rw_lock);

			//printk("[TP9930_READ_REG] reg=0x%02X, value=0x%02X\n", dev_reg.reg_addr, dev_reg.value);

			if (copy_to_user(argp, &dev_reg, sizeof(tp9930_register)))
			{
            	return -EPERM;
			}
		}
	...
	case TP9930_WRITE_REG:
		{
			...
			sensor_write(cci_drv.sd, dev_reg.reg_addr, dev_reg.value);
			
			up(&tp9930_rw_lock);

			//printk("[TP9930_WRITE_REG] reg=0x%02X, value=0x%02X\n", dev_reg.reg_addr, dev_reg.value);
		}
		break;

	case TP9930_GET_VLOSS:
		{
			down(&tp9930_rw_lock);

			if (tp9930_get_vloss(&dev_vloss.video_loss) != 0)
			{
				up(&tp9930_rw_lock);
				return -EINVAL;
			}
			...
			if (copy_to_user(argp, &dev_vloss, sizeof(tp9930_vloss)))
			{
            	return -EPERM;
			}
		}
	...
	case TP9930_IMAGE_SET:
		{
			if (copy_from_user(&dev_image, argp, sizeof(tp9930_image_set)))
			{
            	return -EINVAL;
			}
			...
			if (tp9930_set_image(&dev_image) != 0x0)
			{
				up(&tp9930_rw_lock);
				return -EINVAL;
			}
	...
	case TP9930_POWER_SET:
		{
			if (copy_from_user(&dev_power, argp, sizeof(tp9930_power_set)))
			{
				return -EINVAL;
			}

			if (dev_power.power_set == 0)
			{
...
static ssize_t get_det_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int val = 0;
	int index;
...
#ifdef DUMP_TEST
static ssize_t get_tp9930_dump0_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    //ssize_t val;
    data_type  value_ = 0;
    int i= 0;
...
static ssize_t get_tp9930_dump1_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    //ssize_t val;
    data_type  value_ = 0;
    int i= 0;
...
static ssize_t get_tp9930_dump2_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	//ssize_t val;
	data_type  value_ = 0;
	int i= 0;
...
static ssize_t get_tp9930_dump3_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	//ssize_t val;
	int i= 0;
...
static int tp9930_sensor_det_init(struct i2c_client *client)
{
	...
	if (np == NULL) {
		sensor_err("can not find the tp9930_detect node, will not enable detect kthread\n");
		return -1;
	}
...
static int sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sensor_init_controls(sd, &sensor_ctrl_ops);
        ...
        s_info->fmt_pt 	= &sensor_formats[0];
        s_info->win_pt 	= &sensor_win_sizes[0];
        s_info->fmt_num 	= N_FMTS;
        s_info->win_size_num = N_WIN_SIZES;
        s_info->sensor_field = V4L2_FIELD_NONE;
	...
	sensor_indetect.dev = device_create(sensor_indetect.sensor_class, NULL,
...
free_class:
    ...
    for (i--; i >= 0; i--)
    {
		device_remove_file(sensor_indetect.dev, &detect_dev_attrs[i]);
    }
    #endif
